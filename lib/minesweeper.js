...
const bombCount = (flagCount) => {
  ...
  if (bombCheckCounter === counter) { setTimeout(() => { flash.innerHTML = "<div class='alert alert-success alert-dismissible fade show' role='alert'><strong>WINNER!</strong> You have successfully found every mine!<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>"; }, 100); }
  ...
  allSquares.forEach((square) => {
    ...
    square.addEventListener("click", (event) => {
      if (square.getAttribute("mine") === "true") {
        ...
        setTimeout(() => { flash.innerHTML = "<div class='alert alert-danger alert-dismissible fade show' role='alert'>BOOOOOOOM! <strong>Game Over!</strong> 😥 Reload to reset & try again...<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button></div>"; }, 100);
      }
    });
  });
};
