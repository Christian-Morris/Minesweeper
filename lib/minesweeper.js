...
// - game timer
// - rails app for:
// -> multiplayer (choose game mode at start, enter multiplayer room, give everyone the same grid (after saving it to DB) by using websocket/actioncable? )
// -> leaderboard
...
// 6 - check for number of bombs total vs number found, declare victory if you find them all (reveal all unopened tiles)
const bombCount = (flagCount) => {
  ...
  if (bombCheckCounter === counter) {
    ...
    allSquares.forEach((cell) => {
      if (cell.classList.contains("unopened") === true && cell.classList.contains("flagged") === false) {
        cell.classList.remove("unopened");
        if (cell.getAttribute("neighbourNumber") > 0) {
          cell.classList.add(`mine-neighbour-${cell.getAttribute("neighbourNumber")}`); // assigns the right svg for the corresponding neighbour number
        } else { cell.classList.add("opened"); }
       }
      });
  }
...
// marking the submit button as ready
const difficulty = document.querySelector("#difficulty");
difficulty.addEventListener("change", () => {
  const gridSize = document.querySelector("#grid-size").value
  if  (gridSize < 51 && gridSize > 1) {
    document.querySelector("#submit").classList.add("submit-valid");
  }
})
gridSize.addEventListener("change", () => {
  if  (gridSize.value < 51 && gridSize.value > 1) {
    if (difficulty.value == "easy" || difficulty.value =="medium" || difficulty.value == "hard") {
      document.querySelector("#submit").classList.add("submit-valid");
    }
  } else {
    document.querySelector("#submit").classList.remove("submit-valid");
  }
})
...
setInterval(() => {
 if (timerOn === "yes") {
   ...
   const timer = ((currentTime - startTime)/1000).toFixed(2);
   if ( timer < 10) {
     timerDiv.innerHTML = `0${timer}`;
